...
jQuery(document).ready(function($) {
  ...
  $("#crm_dashboard").submit(function(e) {
    ...
    //show_loading_bar(40);
    $searchFilter.UsersID = $("#crm_dashboard")
      .find("[name='UsersID[]']")
      .val();
    $searchFilter.CurrencyID = $("#crm_dashboard")
  ...
  $searchFilter.CurrencyID = $(
    "#crm_dashboard select[name='CurrencyID']"
  ).val();

  //opportunites grid start
  function Getopportunities() {
    if (CrmDashboardOpportunities === 0) {
      return false;
    }
    data_table = $("#opportunityGrid").dataTable({
      ...
      fnServerParams: function(aoData) {
        $searchFilter.AccountOwner = $(
          "#crm_dashboard [name='UsersID[]']"
        ).val();
        $searchFilter.CurrencyID = $(
          "#crm_dashboard select[name='CurrencyID']"
        ).val();
      ...
      aoColumns: [
        ...
        },
        {
          ...
          mRender: function(id, type, full) {
            return (
              '<input type="text" class="knob" data-min="0" data-max="5" data-width="40" data-height="40" name="Rating" value="' +
              full[17] +
              '" />'
            );
          }
          ...
          mRender: function(id, type, full) {
            ...
            if (Opportunity_edit == 1) {
              action +=
                ...
                full[2] +
                '" class="edit-deal btn btn-default"><i class="entypo-pencil"></i></button>';
            }
        ...
        if (data1 != "") {
          ...
          if (dataObj.data.length > 0) {
            ...
            $("#crmdpipeline1").highcharts({
              ...
              tooltip: {
                headerFormat:
                  '<span style="color:{series.color};">&#9679;</span>&nbsp;<span style="padding:0">{point.key}:{point.y}</span>',
                pointFormat: "",
                footerFormat: "",
                shared: true,
                useHTML: true
  ...
  function loadingUnload(table, bit) {
    var card = jQuery(table).closest(".card");
    if (bit == 1) {
      blockUI(card);
      card.addClass("reloading");
    } else {
  ...
  function GetSalesData() {
    if (CrmDashboardSalesOpportunity === 0) {
      return false;
    }
    //////////////////////////////////
  ...
  function GetSalesDataAccountManager() {
    ...
    var ListType = $(
      "#crm_dashboard_Sales_Manager [name='ListType']:checked"
    ).val();
    $.ajax({
      type: "POST",
            ...
            $("#crmdSalesManager1").highcharts({
              ...
              tooltip: {
                ...
                formatter: function() {
                  ...
                  var duedate = $(
                    "#crm_dashboard_Sales_Manager [name='Duedate']"
                  ).val();
                  if (dataObj.CurrencyCode !== null) {
                    currency_sign = dataObj.CurrencyCode;
                  }
                  return (
                    ...
                    '" duedate="' +
                    duedate +
                    '" class="click_revenue_diagram" style="color:' +
                    this.color +
                    ';">&#9679;' +
                    name_user +
                    "</a> " +
  ...
  $(document).on(
    "click",
    "#board-start ul.sortable-list li button.edit-deal,#opportunityGrid .edit-deal",
    function(e) {
      if (CrmDashboardOpportunities == 1) {
        ...
        for (var i = 0; i < opportunity.length; i++) {
          var val = rowHidden
            .find('input[name="' + opportunity[i] + '"]')
            .val();
          var elem = $(
            '#edit-opportunity-form [name="' + opportunity[i] + '"]'
          );
          ...
          if (select.indexOf(opportunity[i]) != -1) {
            if (opportunity[i] == "TaggedUsers") {
              ...
              $('#edit-opportunity-form [name="TaggedUsers[]"]').select2(
                "val",
                taggedUsers.split(",")
              );
            } else {
  ...
  $(document).on(
    "click",
    "#board-start ul.sortable-list li button.edit-deal,#taskGrid .edit-deal",
    function(e) {
      e.stopPropagation();
      if ($(this).is("button")) {
        var rowHidden = $(this).prev("div.hiddenRowData");
      ...
      for (var i = 0; i < task.length; i++) {
        ...
        if (select.indexOf(task[i]) != -1) {
          if (task[i] == "TaggedUsers") {
            $('#edit-task-form [name="' + task[i] + '[]"]').select2(
              "val",
              val.split(",")
            );
          } else {
  ...
  $("#edit-opportunity-form").submit(function(e) {
    ...
    var opportunityID = $("#" + formid)
      .find('[name="OpportunityID"]')
      .val();
  ...
  function biuldSwicth(container, formID, checked) {
    ...
    make +=
      '<input name="opportunityClosed" value="' +
      OpportunityClose +
      '" ' +
      checked +
      ' type="checkbox">';
