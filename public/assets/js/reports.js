...
function loading(table, bit) {
  var card = jQuery(table).closest(".loading");
  if (bit == 1) {
    blockUI(card);
...
function getAnalysisData(chart_type, submitdata) {
  ...
  $.ajax({
    ...
    success: function(result) {
      ...
      $("." + chart_type + "-call-count-pie-chart").sparkline(
        data.CallCountVal.split(","),
        {
          type: "pie",
          width: "200",
          height: "200",
          ...
          tooltipFormat:
            '<span style="color: {{color}}">&#9679;</span> {{offset:names}} ' +
            HTML_LBL.total_calls +
            "({{value}})",
          tooltipValueLookups: {
      ...
      $("." + chart_type + "-call-count-pie-chart")
        .parent()
        .parent()
        .find(".call_count_desc")
        .html(data.CallCountHtml);
      $("." + chart_type + "-call-cost-pie-chart").sparkline(
        data.CallCostVal.split(","),
        {
          type: "pie",
          width: "200",
          height: "200",
      ...
      $("." + chart_type + "-call-cost-pie-chart")
        .parent()
        .parent()
        .find(".call_cost_desc")
        .html(data.CallCostHtml);
      $("." + chart_type + "-call-minutes-pie-chart").sparkline(
        data.CallMinutesVal.split(","),
        {
          type: "pie",
          width: "200",
          height: "200",
      ...
      $("." + chart_type + "-call-minutes-pie-chart")
        .parent()
        .parent()
        .find(".call_minutes_desc")
        .html(data.CallMinutesHtml);
    }
  });
}
function set_search_parameter(submit_form) {
  ...
  $searchFilter.EndDate =
    $(submit_form)
      ...
      .val() + end_time;
  $searchFilter.UserID = $(submit_form)
    .find("[name='UserID']")
    .val();
  $searchFilter.Admin = $(submit_form)
    .find("input[name='Admin']")
    .val();
  $searchFilter.chart_type = $(submit_form)
    .find("input[name='chart_type']")
    .val();
  $searchFilter.AccountID = $(submit_form)
    .find("[name='AccountID']")
    .val();
  $searchFilter.CompanyGatewayID = $(submit_form)
    .find("[name='CompanyGatewayID']")
    .val();
  $searchFilter.CountryID = $(submit_form)
    .find("[name='CountryID']")
    .val();
  $searchFilter.Prefix = $(submit_form)
    .find("input[name='Prefix']")
    .val();
  $searchFilter.TrunkID = $(submit_form)
    .find("[name='TrunkID']")
    .val();
  $searchFilter.CurrencyID = $(submit_form)
    .find("[name='CurrencyID']")
    .val();
  $searchFilter.TimeZone = $(submit_form)
    .find("[name='TimeZone']")
    .val();
  $searchFilter.CDRType = $(submit_form)
    .find("[name='CDRType']")
    .val();
  $searchFilter.ResellerOwner = $(submit_form)
    .find("[name='ResellerOwner']")
    .val();
  $searchFilter.tag = $(submit_form)
    .find("[name='tag']")
    .val();
}
...
function reloadCharts(table_id, pageSize, $searchFilter) {
  ...
  if (
    typeof hidecallmonitor == "undefined" &&
    $searchFilter.chart_type != "mdn" &&
    $searchFilter.chart_type != "ldc" &&
    $searchFilter.chart_type != "mec"
  ) {
...
function loadTable(table_id, pageSize, $searchFilter) {
  ...
  data_table = $(table_id).dataTable({
    ...
    sDom:
      "<'row'<'col-xs-6 col-left'l><'col-xs-6 col-right'<'export-data'T>f>r>t<'row'<'col-xs-6 col-left'i><'col-xs-6 col-right'p>>",
    aaSorting: [[1, "desc"]],
    aoColumns: [
      {
        bSortable: true,
        mRender: function(id, type, full) {
          ...
          if (
            $searchFilter.chart_type == "trunk" ||
            $searchFilter.chart_type == "prefix"
          ) {
          ...
          if (
            $searchFilter.chart_type != "destination" &&
            $searchFilter.chart_type != "description"
          ) {
            jQuery.each($searchFilter, function(index, item) {
              var cdr_query_parama = [
                "prefix",
                "StartDate",
                "EndDate",
                "AccountID",
            ...
            output = output.replace(
              "{url}",
              cdr_url + "?" + chart_type_param.slice(0, -1)
            );
      ...
      if (end > 0) {
        ...
        $(
          $(row)
            .children()
            .get(0)
        ).html("<strong>" + TABLE_TOTAL + "</strong>");
        $(
          $(row)
            .children()
            .get(1)
        ).html("<strong>" + TotalCall + "</strong>");
        $(
          $(row)
            .children()
            .get(2)
        ).html("<strong>" + TotalDuration + "</strong>");
        if (TotalCost) {
          $(
            $(row)
              .children()
              .get(3)
          ).html("<strong>" + TotalCost + "</strong>");
        }
        if (TotalMargin) {
          $(
            $(row)
              .children()
              .get(6)
          ).html("<strong>" + TotalMargin + "</strong>");
        }
      } else {
        $(table_id)
          .find("tfoot")
