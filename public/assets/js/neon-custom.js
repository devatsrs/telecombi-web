...
(function($, window, undefined) {
  ...
  $(document).ready(function() {
    ...
    public_vars.$horizontalMenu = public_vars.$pageContainer.find(
      "header.navbar"
    );
    public_vars.$sidebarMenu = public_vars.$pageContainer.find(".sidebar-menu");
    public_vars.$mainMenu = public_vars.$sidebarMenu.find("#main-menu");
    public_vars.$mainContent = public_vars.$pageContainer.find(".main-content");
    public_vars.$sidebarUserEnv = public_vars.$sidebarMenu.find(
      ".sidebar-user-info"
    );
    public_vars.$sidebarUser = public_vars.$sidebarUserEnv.find(".user-link");
    public_vars.$sidebarUser = public_vars.$body.addClass("loaded");
    ...
    if ($.isFunction($.fn.slimScroll)) {
      $(".scrollable").each(function(i, el) {
        ...
        $this.css({ maxHeight: "" }).slimScroll({
          ...
          alwaysVisible:
            parseInt(attrDefault($this, "autohide", 1), 10) == 1 ? false : true
        });
      });
    ...
    $(".card-heading").each(function(i, el) {
      ...
      $body.wrap('<div class="card-body with-table"></div>');
    ...
    $("body")
      .on(
        ...
        function(ev) {
          ...
          var $this = jQuery(this).closest(".card");
      ...
      .on(
        ...
        function(ev) {
          ...
          t.append(TweenMax.to($card, 0.2, { css: { scale: 0.95 } }));
          t.append(
    ...
    function addUserPic(opt) {
      ...
      if (!optimage) {
        ...
        var $opt = $(
          '<span class="userName"><i class="entypo-plus"></i>' +
            $(opt.element).text() +
            "</span>"
        );
        return $opt;
      ...
      $(".daterange").on("apply.daterangepicker", function(ev, picker) {
        $(this).val(
          picker.startDate.format(
            attrDefault($(this), "format", "MM/DD/YYYY")
          ) +
            " - " +
            picker.endDate.format(attrDefault($(this), "format", "MM/DD/YYYY"))
        );
      });
    ...
    $("#user_language.language-selector .dropdown-toggle img").attr(
      "src",
      $("#user_language ul li.active img").attr("src")
    );
    $("#user_language.language-selector .dropdown-toggle span").html(
      $("#user_language ul li.active span").html()
    );
    if (
      typeof customer_alignment != "undefined" &&
...
function format(state) {
  ...
  return (
    "<div class='lang_div'><img class='lang_flag' src='" +
    baseurl +
    "/assets/images/flag/" +
    img +
    "' />" +
...
function buildselect2(el) {
    ...
    opts = {
      ...
      formatResult: function(item) {
        if (item.id == "select2-add") {
          return (
            '<span class="select2-add"><i class="entypo-plus-circled"></i>' +
            item.text +
            "</span>"
          );
        }
  ...
  if ($this.hasClass("small")) {
    opts["minimumResultsForSearch"] = attrDefault(
      $this,
      "allowClear",
      Infinity
    );
  ...
  if ($this.hasClass("select2add")) {
    $this.prepend(
      '<option value="select2-add" disabled="disabled">Add</option>'
    );
  }
...
function fit_main_content_height() {
  ...
  // if (
  //   public_vars.$sidebarMenu.length &&
  //   public_vars.$sidebarMenu.hasClass("fixed") == false
  // ) {
...
function continueWrappingPanelTables() {
  var $tables = jQuery(".card-body.with-table + table");

  if ($tables.length) {
    $tables.wrap('<div class="card-body with-table"></div>');
    continueWrappingPanelTables();
  }
}
...
function ShowToastr(type, message) {
  var opts = {
    ...
    debug: false,
    positionClass:
      rtl() || public_vars.$pageContainer.hasClass("right-sidebar")
        ? "toast-top-left"
        : "toast-top-right ",
    onclick: null,
    showDuration: "300",
  ...
  TableTools.BUTTONS.download = {
    ...
    fnClick: function(nButton, oConfig) {
      ...
      for (var i = 0; i < data_table_extra_params.length; i++) {
        sUrlAddition +=
          data_table_extra_params[i].name +
          "=" +
          data_table_extra_params[i].value +
          "&";
      }
      if (oConfig.sButtonClass == "save-collection sageexport") {
        if (
          confirm(
            "Do you want to change the status of selected invoices to Paid?"
          )
        ) {
...
reloadJobsDrodown = function(reset) {
  ...
  if (customer[0].customer != 1) {
    $.get(baseurl + "/jobs/loadDashboardJobsDropDown?reset=" + reset, function(
      data
    ) {
...
checkFailingCronJob = function() {
  ...
  if (
    (diffMins > timeDelay || isFirstTime) &&
    typeof customer[0].customer != "undefined" &&
    ...
    $.get(baseurl + "/cronjobs/check_failing", function(response) {
      if (typeof response.message != "undefined") {
        ...
        if (response.message == "") {
          ...
          $(".notifications.cron_jobs.dropdown")
            .find("#failing_placeholder")
            .addClass("hidden");
        } else {
...
reloadMsgDrodown = function(reset) {
  return false;
  if (customer[0].customer != 1) {
    $.get(baseurl + "/loadDashboardMsgsDropDown?reset=" + reset, function(
      data
    ) {
...
$(document).on("select2-open", ".select2add", function(e) {
  ...
  var modal = $("#" + self.attr("data-modal"));
  var forms = modal.find("form");
  $('select[data-type="' + self.attr("data-type") + '"]').attr(
    "data-active",
    0
  );
...
function setDefaultValue(type, field) {
  ...
  defaultValue.invoice_template = {
    CompanyLogoUrl: { src: "http://placehold.it/250x100" }
  };
  ...
  defaultValue.ticket_bulk_option = {
    Type: 0,
    Status: 0,
    Priority: 0,
    Group: 0,
...
$(document)
  .on("mouseover", ".shortname", function() {
...
try {
  if (
    typeof customer[0].customer != "undefined" &&
    customer[0].customer != 1 &&
    $(".notifications.cron_jobs.dropdown").has("#failing_placeholder").length >
      0
  ) {
