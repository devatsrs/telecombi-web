...
(function($, doc, global) {
  ...
  UI.fn = function(command, options) {
    ...
    return this.each(function() {
      ...
      if (!data)
        $this.data(
          component,
          (data = new UI[component](this, method ? undefined : options))
        );
  ...
  UI.support.requestAnimationFrame =
  ...
  UI.support.touch =
    ...
    (global.navigator["pointerEnabled"] &&
      global.navigator["maxTouchPoints"] > 0) || //IE >=11
    false;
...
(function($, UI) {
  ...
  // init code
  $(document).on("click.buttonradio.uikit", "[data-uk-button-radio]", function(
    e
  ) {
    var ele = $(this);

    if (!ele.data("buttonRadio")) {
      var obj = new ButtonRadio(
        ele,
      ...
      if (this.options.mode == "click" || UI.support.touch) {
        this.element.on("click", function(e) {
          ...
          if (!$this.element.hasClass("uk-open")) {
            ...
            if (
              $target.is("a") ||
              !$this.element.find(".uk-dropdown").find(e.target).length
            ) {
  ...
  $.extend(Dropdown.prototype, {
    ...
    checkDimensions: function() {
      ...
      var dropdown = this.dropdown
          .css("margin-" + $.UIkit.langdirection, "")
          .css("min-width", ""),
      ...
      if (this.centered) {
        dropdown.css(
          "margin-" + $.UIkit.langdirection,
          (parseFloat(width) / 2 - dropdown.parent().width() / 2) * -1
        );
      ...
      if (this.justified && this.justified.length) {
        ...
        if ($.UIkit.langdirection == "right") {
          var right1 = boundarywidth - (this.justified.offset().left + jwidth),
            right2 =
              boundarywidth - (dropdown.offset().left + dropdown.outerWidth());

          dropdown.css("margin-right", right1 - right2);
        } else {
          dropdown.css(
            "margin-left",
            this.justified.offset().left - offset.left
          );
        }
    ...
    function(e) {
      ...
      if (!ele.data("dropdown")) {
        var dropdown = new Dropdown(
          ele,
          UI.Utils.options(ele.data("uk-dropdown"))
        );
...
(function($, UI, $win) {
  ...
  var active = false,
    ...
    Modal = function(element, options) {
      ...
      this.element
        .on("click", ".uk-modal-close", function(e) {
          e.preventDefault();
          $this.hide();
        })
        .on("click", function(e) {
  ...
  $.extend(Modal.prototype, {
    ...
    resize: function() {
      ...
      var modalwidth = parseInt(this.dialog.css("width"), 10),
        inview =
          modalwidth +
            parseInt(this.dialog.css("margin-left"), 10) +
            parseInt(this.dialog.css("margin-right"), 10) <
          $win.width();

      this.dialog.css(
        "margin-left",
        modalwidth && inview ? -1 * Math.ceil(modalwidth / 2) : ""
      );
    },
  ...
  ModalTrigger.alert = function(content, options) {
    ModalTrigger.dialog(
      ...
      $.extend({ bgclose: false, keyboard: false }, options)
    ).show();
  };
  ...
  function setContent(content, modal) {
    ...
    if (typeof content === "object") {
      ...
      content = $("<div></div>").html(
        "$.UIkitt.modal Error: Unsupported data type: " + typeof content
      );
    }
    ...
    Offcanvas = {
      show: function(element) {
        ...
        var doc = $("html"),
          ...
          dir =
            (bar.hasClass("uk-offcanvas-bar-flip") ? -1 : 1) * (rtl ? -1 : 1),
          scrollbar =
            dir == -1 && $win.width() < window.innerWidth
        ...
        doc
          .css(
            ...
            (rtl ? -1 : 1) * ((bar.outerWidth() - scrollbar) * dir)
          )
      ...
      hide: function(force) {
        var doc = $("html"),
          card = $(".uk-offcanvas.uk-active"),
          rtl = $.UIkit.langdirection == "right",
          bar = card.find(".uk-offcanvas-bar:first");

        if (!card.length) return;

        if ($.UIkit.support.transition && !force) {
          doc
            .one($.UIkit.support.transition.end, function() {
              doc.removeClass("uk-offcanvas-page").attr("style", "");
              card.removeClass("uk-active");
      ...
      if ($li.data("list-container")) {
        if (noanimation) {
          $li
            .data("list-container")
            .stop()
            .height($li.hasClass("uk-open") ? "auto" : 0);
        } else {
          $li
            .data("list-container")
            .stop()
            .animate({
              height: $li.hasClass("uk-open")
                ? getHeight($li.data("list-container").find("ul:first"))
                : 0
            });
        }
      }
...
(function($, UI) {
  ...
  $.extend(Switcher.prototype, {
    show: function(tab) {
      tab = isNaN(tab)
        ? $(tab)
        : this.element.find(this.options.toggler).eq(tab);
...
(function($, UI) {
  ...
  var Tab = function(element, options) {
    ...
    if (this.element.hasClass("uk-tab-flip"))
      dropdown.addClass("uk-dropdown-flip");

    this.element.find("a").each(function(i) {
      var tab = $(this).parent(),
        item = $(
          '<li><a href="javascript:void(0);">' + tab.text() + "</a></li>"
        ).on("click", function(e) {
    ...
    Search = function(element, options) {
      ...
      this.input.on({
        keydown: function(event) {
          $this.form[$this.input.val() ? "addClass" : "removeClass"](
            $this.options.filledClass
          );
      ...
      this.dropdown = $(
        '<div class="uk-dropdown uk-dropdown-search"><ul class="uk-nav uk-nav-search"></ul></div>'
      )
        .appendTo(this.form)
        .find(".uk-nav-search");
  ...
  $.extend(DefaultRenderer.prototype, {
    ...
    suggest: function(data) {
      ...
      if (this.options.msgResultsHeader) {
        $("<li>")
          .addClass(
            this.options.resultsHeaderClass + " " + this.options.skipClass
          )
          .html(this.options.msgResultsHeader)
          .appendTo(this.search.dropdown);
      }

      if (data.results && data.results.length > 0) {
        $(data.results).each(function(i) {
          var item = $('<li><a href="#">' + this.title + "</a></li>").data(
            "choice",
            this
          );
        ...
        if (this.options.msgMoreResults) {
          $("<li>")
            .addClass("uk-nav-divider " + $this.options.skipClass)
            .appendTo($this.dropdown);
          $("<li>")
            .addClass($this.options.moreResultsClass)
            .html('<a href="#">' + $this.options.msgMoreResults + "</a>")
            .appendTo($this.search.dropdown)
            .on(events);
        }
        ...
        $("<li>")
          .addClass(this.options.noResultsClass + " " + this.options.skipClass)
          .html("<a>" + this.options.msgNoResults + "</a>")
          .appendTo($this.search.dropdown);
      }
    }
  });
...
(function($, UI) {
  ...
  var scrollspynavs = [],
    checkScrollSpyNavs = function() {
      for (var i = 0; i < scrollspynavs.length; i++) {
        UI.support.requestAnimationFrame.apply(window, [
  ...
  $win
    .on("scroll", fnCheck)
    .on("resize orientationchange", UI.Utils.debounce(fnCheck, 50));
