...
(function ($, window, undefined) {
  ...
  $(document).ready(function () {
    ...
    neonLogin.$container = $("#form_customer_login");
    ...
    neonLogin.$container.validate({
      ...
      submitHandler: function (ev) {
        ...
        setTimeout(function () {
          ...
          //alert( JSON.stringify(neonLogin.$container.serializeArray()));
          var postData = $("#form_customer_login").serializeArray();

          // Send data to the server
          $.ajax({
            url: baseurl + "/customer/dologin",
            method: "POST",
            dataType: "json",
            data: postData /*{
                         email: $("input#email").val(),
    ...
    neonLogin.$sacontainer = $("#super_admin_form_login");

    neonLogin.$sacontainer.validate({
      rules: {
        Company: {
          required: true,
        },
      ...
      submitHandler: function (ev) {
        ...
        setTimeout(function () {
          ...
          // Send data to the server
          $.ajax({
            url: baseurl + "/do_login_for_super_admin",
            method: "POST",
            dataType: "json",
            data: {
    ...
    neonLogin.$login_progressbar_indicator = $(
      ".login-progressbar-indicator h3"
    );
    neonLogin.$login_progressbar = neonLogin.$body.find(
    ...
    $.extend(neonLogin, {
      setPercentage: function (pct, callback) {
        ...
        var o = {
          pct: parseInt(
            (neonLogin.$login_progressbar.width() /
              neonLogin.$login_progressbar.parent().width()) *
              100,
            10
          ),
        };
      ...
      resetProgressBar: function (display_errors) {
        ...
        setTimeout(function () {
          TweenMax.to(neonLogin.$container, 0.6, {
            css: { opacity: 1 },
            onComplete: function () {
          ...
          if (display_errors) {
            ...
            TweenMax.to($errors_container, 0.45, {
              css: { height: height },
              onComplete: function () {
    ...
    if (is_lockscreen) {
      ...
      neonLogin.$lockscreen_progress_indicator = neonLogin.$container.find(
      ...
      neonLogin.lockscreen_progress_canvas = neonLogin.$lockscreen_progress_canvas.get(
        0
      );
    ...
    $("#user_language ul li").click(function () {
      window.location =
        baseurl + "/customer/login?lang=" + $(this).attr("lang-key");
      return false;
    });
    ...
    if ($("#user_language ul li.active").length) {
      var page_url =
        baseurl +
        "/customer/login?lang=" +
        $("#user_language ul li.active").attr("lang-key");
