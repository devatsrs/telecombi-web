...
$(document).ready(function(){
    ...
    /** CreditNotes Usage Functions
    * */

    function getCalculateCreditNotesByProduct(product_type,productID,AccountID,qty,callback){
        ...
        var _url = baseurl + '/creditnotes/calculate_total';
        $.post( _url, post_data, callback, "json" );
    }

    function getCalculateCreditNotesBySubscription(product_type,productID,AccountID,qty,callback){
		var AccountBillingClassID = $('#AccountBillingClassID').val();
        post_data = {"product_type":product_type,"product_id":productID,"account_id":AccountID,"qty":qty,"BillingClassID":AccountBillingClassID};
        var _url = baseurl + '/creditnotes/calculate_total';
        $.post( _url, post_data, callback, "json" );
    ...
    function getCalculateCreditNotesByDuration(product_type,productID,AccountID,qty,start_date,end_date,CreditNotesDetailID,callback){
		var AccountBillingClassID = $('#AccountBillingClassID').val();
        post_data = {"product_type":product_type, "product_id":productID,"account_id":AccountID,"qty":qty,"start_date":start_date,"end_date":end_date,"CreditNotesDetailID":CreditNotesDetailID,"BillingClassID":AccountBillingClassID};
        var _url = baseurl + '/creditnotes/calculate_total';
        $.post( _url, post_data, callback, "json" );
    }
    /** -----------------------------------*/
    ...
    function getCreditNotesUsage(creditnotes_id,callback){
        post_data = { "creditnotes_id":creditnotes_id };
        var _url = baseurl + '/creditnotes/'+creditnotes_id+'/print_preview';
        $.get( _url, post_data, callback, "html" );
    }
    function sendCreditNotes(creditnotes_id,post_data,callback){
        //post_data = { "creditnotes_id":creditnotes_id };
        var _url = baseurl + '/creditnotes/'+creditnotes_id+'/send';
        $.post( _url, post_data, callback, "json");
    }

    $("#CreditNotesTable").delegate( '.product_dropdown' ,'change',function (e) {
        var $this = $(this);
        ...
        var CreditNotesDetailID = $row.find('.CreditNotesDetailID').val();
        ...
        if( typeof productID!="undefined" && productID != ''  && parseInt(AccountID) > 0 ) {
            try{
                ...
                if(product_types[selected_product_type] == USAGE ) {
                    ...
                    $('#add-new-creditnotes-duration-form').trigger('reset');
                    $('#add-new-creditnotes-duration-form .save.btn').button('reset');

                    $('#add-new-modal-creditnotes-duration').modal('show');
                    $('#add-new-creditnotes-duration-form').submit(function(e){
                        e.preventDefault();
                        setTimeout(function(e){
                            start_date = $('#add-new-creditnotes-duration-form input[name=start_date]').val();
                            end_date = $('#add-new-creditnotes-duration-form input[name=end_date]').val();
                            start_time = $('#add-new-creditnotes-duration-form input[name=start_time]').val();
                            end_time = $('#add-new-creditnotes-duration-form input[name=end_time]').val();
                            CreditNotesDetailID = parseInt(CreditNotesDetailID);
                            ...
                            getCalculateCreditNotesByDuration(selected_product_type,productID,AccountID,1,start_date,end_date,CreditNotesDetailID,function(response){
                                $('#add-new-creditnotes-duration-form').trigger('reset');
                                $('#add-new-creditnotes-duration-form .save.btn').button('reset');
                                if(response.status =='success'){
                                    $('#add-new-modal-creditnotes-duration').modal('hide');
                    ...
                    getCalculateCreditNotesBySubscription('subscription',productID,AccountID,1,function(response){
                    ...
                    getCalculateCreditNotesByProduct('item',productID,AccountID,1,function(response){
                        //console.log(response);
                        if(response.status =='success'){
                            //  $row.find("select.TaxRateID").val(response.product_tax_rate_id).trigger("change");
                            //$row.find("select.TaxRateID2").val(response.product_tax_rate_id).trigger("change");
    ...
    $("#CreditNotesTable").delegate( '.Price , .Qty , .Discount, .TaxRateID, .TaxRateID2' ,'change',function (e) {
        var $this = $(this);
        var $row = $this.parents("tr");
        cal_line_total($row);
        calculate_total();
    ...
    $("input[name=discount]").change(function (e) {
	 ...
	 $(".creditnotes_tax_add").click(function (e) {
	   e.preventDefault();
	    var index_count = $('.all_tax_row').length+1;
        var creditnotes_tax_html_final  = '<tr class="all_tax_row CreditNotesTaxestr'+index_count+' ">'+creditnotes_tax_html+"</tr>";
		$('.gross_total_creditnotes').before(creditnotes_tax_html_final);
		$('select.select2').addClass('visible');
        $('select.select2').select2();
		calculate_total();
    });

	 $(document).on('click','.creditnotes_tax_remove', function(e){
	    e.preventDefault();
    ...
    $('#add-row').on('click', function(e){
        ...
        $('#CreditNotesTable > tbody').append(itemrow);
        nicescroll();
		$("textarea.autogrow").autosize();
    });

    $('#CreditNotesTable > tbody').on('click','.remove-row', function(e){
        e.preventDefault();
        var row = $(this).parent().parent();
        row.remove();
        calculate_total();
	...
	$(document).on('change','.CreditNotesTaxesFld', function(e){
        e.preventDefault();
        var row = $(this).parent().parent();
       // row.find('.CreditNotesTaxesValue').val($(this).val());
        calculate_total();
    });
    ...
    function calculate_total(){
        ...
        $('#CreditNotesTable tbody tr td .TaxAmount').each(function(i, el){
            var $this = $(el);
            if($this.val() != ''){
		...
		$('#CreditNotesTable tbody tr td select.Taxentity').each(function(i, el){
            var $this 	=	 $(el);
		...
		$('.tax_rows_creditnotes').remove();


		Tax_type.forEach(AddTaxRows);
		function AddTaxRows(value, index) {
			if(value != null){
				$('.grand_total_creditnotes').before('<tr class="tax_rows_creditnotes"><td>'+Tax_type_title[index]+'</td><td><input class="form-control text-right" readonly="readonly" name="Tax['+index+']" value="'+value.toFixed(decimal_places)+'" type="text">  </td> </tr>');
			}
		}

        $('#CreditNotesTable tbody tr td .LineTotal').each(function(i, el){
            var $this = $(el);
            if($this.val() != ''){
      ...
      //  $(".product_tax_title").text(taxTitle);
        creditnotes_main_total_tax = 0; var taxes_array = new Array();
	   $('.CreditNotesTaxesFld').each(function(index, element) {
		    ...
		    if($this.val() != '' && $this.val() != 0)
			{
				  ...
				  if(flatstatus == 1){
				   ...
				   obj.find('.CreditNotesTaxesValue').val(tax.toFixed(decimal_places));
                creditnotes_main_total_tax = parseFloat(creditnotes_main_total_tax)+parseFloat(tax);
            }
			else
			{
				  var obj 		 =   $(element).parent().parent();
				   obj.find('.CreditNotesTaxesValue').val(0);
			}
    	});
		var gross_total = parseFloat(total)+creditnotes_main_total_tax;
		 $('input[name=GrandTotalCreditNotes]').val(gross_total.toFixed(decimal_places));
...
//    function cal_line_total(obj){
    ...
    $(".send-creditnotes.btn").click( function (e) {
    ...
    $('select.TaxRateID').on( "change",function(e){
        ...
        var rowCount = $('#CreditNotesTable tbody tr').length;
    ...
    $("select[name=AccountID]").change( function (e) {
        url = baseurl + "/creditnotes/get_account_info";
        $this = $(this);
        ...
        if($this.val() > 0){
            ajax_json(url,data,function(response){
                if ( typeof response.status != undefined &&  response.status == 'failed') {
                    ...
                    $("input[name=CreditNotesTemplateID]").val('');
                    $("[name=Terms]").val('');
                    $("[name=FooterTerm]").val('');
                } else {
                    $("#Account_Address").html(response.CreditNotesToAddress);
                    ...
                    $("input[name=CreditNotesTemplateID]").val(response.CreditNotesTemplateID);
                    $("[name=Terms]").val(response.Terms);
                    $("[name=FooterTerm]").val(response.FooterTerm);
					add_invoce_tax(response.AccountTaxRate);
                    CreditNotesTemplateID = response.CreditNotesTemplateID;
                }
				show_summerinvoicetemplate($("[name=Terms]"));
				show_summerinvoicetemplate($("[name=FooterTerm]"));
            });
	...
	$("#AccountBillingClassID").change( function (e) {
        ...
        url = baseurl + "/creditnotes/get_billingclass_info";
        $this = $(this);
	...
	function add_invoce_tax(AccountTaxRate){
		...
		if(AccountTaxRate.length>0){
			AccountTaxRate.forEach(function(entry,index) {
				...
				if(index==0){
					$('.CreditNotesTaxesFldFirst').val(entry);
					var change = $('.CreditNotesTaxesFldFirst');
					change.trigger('change');
				}
				else
				{
				   var creditnotes_tax_html_final  = '<tr class="all_tax_row CreditNotesTaxestr'+index+' ">'+creditnotes_tax_html+"</tr>";
				   $('.gross_total_creditnotes').before(creditnotes_tax_html_final);
				   var current_obj = $('.CreditNotesTaxestr'+index).find('.CreditNotesTaxesFld');
				   current_obj.addClass('CreditNotesTaxesFld'+index);
				   current_obj.val(entry);
				   current_obj.addClass('visible');
				   current_obj.select2();
				   current_obj.trigger('change');
				   // var change = $('.CreditNotesTaxesFld').eq(index+1);
				}
			});
			 //$('select.CreditNotesTaxesFld').addClass('visible');
			 //$('select.CreditNotesTaxesFld').select2();
			 //$('select.CreditNotesTaxesFld').trigger('change');
			 calculate_total();
		}
	}
    //Calculate Total
...
<style>
#CreditNotesTable.table > tbody > tr > td > div > a > span.select2-chosen { width:110px;}
</style>