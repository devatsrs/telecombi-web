            ...
            <div data-collapsed="0" class="card shadow card-primary">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <div class="col-sm-2">
                            <input type="text" name="StartDate" class="form-control datepicker"  data-date-format="yyyy-mm-dd" value="{{$start_date}}"  data-enddate="{{date('Y-m-d')}}"/>
                        </div>
                        <label class="col-sm-1 control-label" for="field-1">End date</label>
                        <div class="col-sm-2">
                            <input type="text" name="EndDate" class="form-control datepicker"  data-date-format="yyyy-mm-dd" value="{{$end_date}}"  data-enddate="{{date('Y-m-d', strtotime('+1 day') )}}"/>
                        </div>
                        <label class="col-sm-1 control-label" for="field-1">Gateway Account</label>
                        <div class="col-sm-2">
                            {{ Form::select('AccountID',$account,$accountid, array("class"=>"select2")) }}
                        </div>
...
var postdata;
    jQuery(document).ready(function ($) {
        ...
        if($searchFilter.AccountID == '' || typeof $searchFilter.AccountID  == 'undefined'){
        ...
        data_table = $("#table-4").dataTable({
            ...
            "bProcessing":true,
            "bServerSide":true,
            "sAjaxSource": baseurl + "/summaryreport/daily_ajax_datagrid",
            "iDisplayLength": parseInt('{{CompanyConfiguration::get('PAGE_SIZE')}}'),
            "sPaginationType": "bootstrap",
            "sDom": "<'row'<'col-xs-6 col-left'l><'col-xs-6 col-right'<'export-data'T>f>r>t<'row'<'col-xs-6 col-left'i><'col-xs-6 col-right'p>>",
            "aaSorting": [[1, 'asc']],
            "fnServerParams": function(aoData) {
                aoData.push({"name":"StartDate","value":$searchFilter.StartDate},{"name":"EndDate","value":$searchFilter.EndDate},{"name":"AccountID","value":$searchFilter.AccountID},{"name":"GatewayID","value":$searchFilter.GatewayID});
                data_table_extra_params.length = 0;
                data_table_extra_params.push({"name":"StartDate","value":$searchFilter.StartDate},{"name":"Export","value":1},{"name":"EndDate","value":$searchFilter.EndDate},{"name":"AccountID","value":$searchFilter.AccountID},{"name":"GatewayID","value":$searchFilter.GatewayID});
            },
             "aoColumns":
            [
                ...
                "aButtons": [
                    {
                        ...
                        "sUrl": baseurl + "/summaryreport/daily_ajax_datagrid", //baseurl + "/generate_xls.php",
